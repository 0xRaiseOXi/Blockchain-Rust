...
impl Blockchain {
    ...
    pub fn create_block(&mut self) {
        ...
        //     self.update_state(sender, recipient, amount, contract);
        // }
    ...
    pub fn update_state(&mut self, sender: String, recipient: String, amount: f64, contact: String) -> Option<f64> {
        if let Some(ref db) = self.database_s {
            ...
            sender_object.subtraction(contact.clone(), amount);
            recipient_object.addition(contact.clone(), amount);
    ...
    pub fn get_balance_account(&self, public_key: String) -> Option<HashMap<String, f64>> {
        if let Some(ref db) = self.database_s {
            match db.get(public_key.as_bytes()) {
                Ok(Some(value)) => {
                    let wallet: Wallet = serde_json::from_slice(&value).unwrap();
                    return wallet.get_all_balance();
                }
                Ok(None) => {
                    println!("Значение не найдено");
                }
